...
#[tauri::command]
pub fn set_audio_device(state: State<'_, AppState>, name: String) -> Result<(), String> {
    *state.selected_device.lock() = Some(name);
    Ok(())
}
...
pub fn get_hotkey(state: State<'_, AppState>) -> String {
    ...
    if mods.contains(tauri_plugin_global_shortcut::Modifiers::CONTROL) { label.push_str("Ctrl + "); }
    if mods.contains(tauri_plugin_global_shortcut::Modifiers::SHIFT) { label.push_str("Shift + "); }
    if mods.contains(tauri_plugin_global_shortcut::Modifiers::ALT) { label.push_str("Alt + "); }
    if mods.contains(tauri_plugin_global_shortcut::Modifiers::SUPER) { label.push_str("Super + "); }
...
pub async fn download_model(app: AppHandle, tier: String) -> Result<(), String> {
    ...
    let (model_url, filename) = match tier.as_str() {
        "realfast" => ("https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-tiny.en.bin", "ggml-tiny.en.bin"),
        "fast" => ("https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-base.en.bin", "ggml-base.en.bin"),
        "standard" => ("https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-small.en.bin", "ggml-small.en.bin"),
        "pro" => ("https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-large-v3-turbo.bin", "ggml-large-v3-turbo.bin"),
    ...
    *state.selected_model.lock() = filename.to_string();
    ...
    if model_path.exists() {
        ...
        app.emit("download-progress", 100).map_err(|e| e.to_string())?;
        return Ok(());
    }
    ...
    let mut file = tokio::fs::File::create(&model_path).await.map_err(|e: std::io::Error| e.to_string())?;
...
pub fn get_onboarding_status(app: AppHandle) -> bool {
    let model_path = app.path().app_data_dir().unwrap().join("model.bin");
    let exists = model_path.exists();
    println!("[DEBUG] Onboarding check: Model exists = {}", exists);
    exists
}

#[tauri::command]
pub fn complete_onboarding(_app: AppHandle) -> Result<(), String> {
    println!("[DEBUG] Onboarding completed and persisted.");
    // In a real app we'd save to a config.json, but model.bin existence is our flag for now
    Ok(())
}
